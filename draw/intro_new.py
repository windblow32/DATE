    ...
    for i in np.unique(labels):
        ...
        if i == 2:
            ax1.scatter(X[mask, 0], X[mask, 1], 
                       ...
                       label=names[int(i)],
                       alpha=1, s=200, edgecolors='white', linewidth=0.5)
        else:
            ax1.scatter(X[mask, 0], X[mask, 1], 
                       c=colors[int(i)], 
                       marker=markers[int(i) % len(markers)],
                       label=names[int(i)],
                       alpha=0, s=50, edgecolors='white', linewidth=0.5)
...
def plot_custom_heterogeneous_data():
    ...
    r = np.random.normal(2, 0.2, 20)
    cluster1 = np.column_stack([r * np.cos(theta) + 1, r * np.sin(theta) + 1])
    
    # 分布2: 分散的线性分布
    x2 = np.random.uniform(-2, 4, 20)
    y2 = 0.6 * x2 + 1 + np.random.normal(0, 0.8, 20)
    cluster2 = np.column_stack([x2, y2])
    
    # 分布3: 另一个紧凑分布
    theta3 = np.random.uniform(0, 2*np.pi, 20)
    r3 = np.random.normal(1.5, 0.25, 20)
    cluster3 = np.column_stack([r3 * np.cos(theta3) - 2, r3 * np.sin(theta3) - 1])
    
    # 合并数据
    X = np.vstack([cluster1, cluster2, cluster3])
    labels = np.hstack([np.zeros(20), np.ones(20), np.ones(20)*2])
...
def plot_individual_distributions(X, labels, colors, names):
    """Generate three separate plots, each showing only one distribution"""
    markers = ['o', 's', '^', 'D', '*', 'p', 'h', 'X']
    
    # Create a figure for each distribution
    for dist_idx in range(3):
        fig, ax = plt.subplots(figsize=(6, 6))
        
        # Plot only the current distribution
        mask = (labels == dist_idx)
        ax.scatter(X[mask, 0], X[mask, 1], 
                  c=colors[dist_idx], 
                  marker=markers[dist_idx % len(markers)],
                  alpha=1, s=50, 
                  label=names[dist_idx],
                  edgecolors='white', 
                  linewidth=0.5)
        
        # Customize the plot
        ax.set_title(f'Distribution {chr(65+dist_idx)}', fontsize=14, fontweight='bold')
        ...
        # Save the figure
        plt.savefig(f'distribution_{chr(65+dist_idx)}.png', dpi=300, bbox_inches='tight')
        plt.close()  # Close the figure to free memory

    print("\nIndividual distribution plots saved as distribution_A.png, distribution_B.png, and distribution_C.png")
...
if __name__ == "__main__":
    ...
    plot_individual_distributions(X_custom, labels_custom, 
