...
# ==== 修改开始：保留 score top3 的数据对象 ====
# 提取所有对象的 score 值
scores = [obj['score'] for obj in data_objects]
# 保留score排名前k个数据对象
k = 5
# 找到 topk 的索引
topk_indices = np.argsort(scores)[-k:][::-1]  # 从高到低排序

# 创建 topk 对象列表
topk_objects = [data_objects[i] for i in topk_indices]

# 打印 topk 的信息
print("\nTop k data objects by score:")
for i, obj in enumerate(topk_objects, 1):
    print(f"{i}. Score: {obj['score']:.4f}, Path: {obj['path']}")
# ==== 修改结束 ====
...
# 验证贪心选择性
# 只处理 topk 的数据对象
for idx, data_obj in tqdm(enumerate(topk_objects), total=len(topk_objects), desc="Processing top k data objects"):
...
print(f"Selected data objects: {len(selected_objects)}/{len(topk_objects)}")
